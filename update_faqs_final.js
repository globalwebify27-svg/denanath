...
async function main() {
  ...
  // Because the previous edit left broken html, we should clear everything after <h3>FAQs</h3> 
  // up to "Contact Us" or just replace the whole section.
  
  let html = department.description;
  const faqStart = html.indexOf('<h3 class="text-xl font-bold text-[#002b5c] mb-4 border-b pb-2">FAQs</h3>');
  const contactStart = html.indexOf('<h3 class="text-xl font-bold text-[#002b5c] mb-4 border-b pb-2">Contact Us</h3>');
  
  if (faqStart !== -1 && contactStart !== -1) {
    // Manually replace everything between <section> for FAQs and the next section.
    // The previous section might not have been correctly closed due to the mangled HTML.
    
    // Instead of raw string replacement, let's use cheerio but carefully remove the broken lists.
    let faqSection = $('h3').filter(function() {
      return $(this).text().trim() === 'FAQs';
    }).closest('section');
    
    if (faqSection.length > 0) {
      faqSection.replaceWith(newFaqHtml);
    } else {
      // If it's not wrapped in a section anymore, it's heavily broken.
      // We will do string replacement.
      let startIdx = html.lastIndexOf('<section', faqStart);
      let endIdx = html.lastIndexOf('<section', contactStart);
      
      let newFullHtml = html.substring(0, startIdx) + newFaqHtml + html.substring(endIdx);
      html = newFullHtml;
      
      const new$ = cheerio.load(html, null, false);
      html = new$.html();
    }
    
    // Clean up any stray <p> tags with inline styles that got left behind
    const final$ = cheerio.load(html, null, false);
    // Any remaining stray text from the broken edit that didn't get removed?
    // The broken elements were next to the <ul>, so if we replace the section, they should be gone.
    
    await prisma.department.update({
      where: { id: department.id },
      data: { description: final$.html() }
    });

    console.log('Successfully updated FAQs and fixed the formatting.');
  } else {
    // Fallback if we can't find it
    console.log("Could not find the exact markers. Using Cheerio replace.");
    ...
    console.log('Successfully updated FAQs using cheerio.');
  }
}
