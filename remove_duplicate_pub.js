...
async function main() {
  ...
  if (settings) {
    ...
    const initialCount = (data.publications || []).length;
    
    // Filter out the duplicate entry
    data.publications = (data.publications || []).filter(pub => {
      // Return false (remove) if it matches the specific duplicate
      const titleMatch = typeof pub.title === 'string' && pub.title.trim() === 'International Nosocomial Infection Control Consortium (INICC) report, data summary of 45 countries for 2012-2017: Device-associated module.';
      const authorsMatch = typeof pub.authorsDate === 'string' && pub.authorsDate.trim() === 'Rosenthal VD, Bat-Erdene I, Gupta D, Belkebir S, Rajhans P, et al.';
      
      // If both match, this is the one to remove
      if (titleMatch && authorsMatch) {
        console.log("Found duplicate publication to remove:", pub);
        return false;
      }
      return true;
    });
    
    const finalCount = data.publications.length;
    console.log(`Removed ${initialCount - finalCount} duplicate publications.`);
    ...
    if (initialCount !== finalCount) {
        ...
        console.log("Successfully removed duplicate and regenerated HTML!");
    } else {
        console.log("Duplicate publication was not found.");
    }
  } else {
