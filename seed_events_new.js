...
async function main() {
  await prisma.siteSetting.upsert({
    ...
    update: { value: JSON.stringify({ events: newEvents }) },
    create: { key: 'page_training_events', value: JSON.stringify({ events: newEvents }) }
  });
  console.log("Seeded page_training_events in DB!");

  const frontendFilePath = path.join(__dirname, 'src/app/(research)/training-events/page.tsx');
  let frontendContent = fs.readFileSync(frontendFilePath, 'utf-8');
  
  // replace fallback array in page.tsx
  const pageRegex = /const fallbackEvents = \[\s*\{[^]*?\}\s*\];/;
  frontendContent = frontendContent.replace(pageRegex, `const fallbackEvents = ${JSON.stringify(newEvents, null, 2)};`);
  fs.writeFileSync(frontendFilePath, frontendContent);
  console.log("Updated fallback in page.tsx!");

  const backendFilePath = path.join(__dirname, 'src/app/admin/(dashboard)/research/training-events/client-form.tsx');
  let backendContent = fs.readFileSync(backendFilePath, 'utf-8');
  
  // replace fallback array in client-form.tsx
  const clientRegex = /initialData\?.events \:\s*\[[^]*?\]\);/;
  backendContent = backendContent.replace(clientRegex, `initialData?.events : ${JSON.stringify(newEvents, null, 2)});`);
  fs.writeFileSync(backendFilePath, backendContent);
  console.log("Updated fallback in client-form.tsx!");
}
