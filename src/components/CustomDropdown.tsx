...
export default function CustomDropdown({ name, options, placeholder = "-- Select --", icon: Icon, required, defaultValue = "", value, className = "", onChange }: any) {
  ...
  return (
    <div className={`relative w-full ${isOpen ? "z-50" : "z-10"}`} ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 bg-white rounded-b-lg shadow-lg border border-slate-200 border-t-0 max-h-60 overflow-y-auto z-50 py-1 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-300 [&::-webkit-scrollbar-thumb]:rounded-full flex flex-col">
