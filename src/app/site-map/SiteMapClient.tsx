...
import { Home, ChevronsRight, ChevronDown, ChevronRight, Map } from "lucide-react";

type DynamicPage = { title: string; slug: string; navbarMenu: string };

export default function SiteMapClient({ dynamicPages = [] }: { dynamicPages?: DynamicPage[] }) {
  ...
  // Group dynamic pages by navbarMenu
  const dynamicGroups: Record<string, DynamicPage[]> = {};
  dynamicPages.forEach(p => {
    const group = p.navbarMenu || "Other";
    if (!dynamicGroups[group]) dynamicGroups[group] = [];
    dynamicGroups[group].push(p);
  });

  // These hardcoded hrefs will be deduplicated from dynamic groups below
  ...
  // Populate hardcoded hrefs for deduplication
  sitemapSections.forEach(s => s.links.forEach(l => hardcodedHrefs.add(l.href)));

  // Add dynamic page groups as extra sections — skip pages already listed in hardcoded sections
  const existingTitles = new Set(sitemapSections.map(s => s.title));
  const extraSections = Object.entries(dynamicGroups)
    .map(([group, pages]) => ({
      title: existingTitles.has(group) ? `${group} (Custom)` : group,
      links: pages
        .filter(p => !hardcodedHrefs.has(`/${p.slug}`))
        .map(p => ({ label: p.title, href: `/${p.slug}` })),
    }))
    .filter(s => s.links.length > 0); // hide empty sections after dedup

  const allSections = [...sitemapSections, ...extraSections];
    ...
    <div className="min-h-screen bg-[#f8fafc] font-sans selection:bg-teal-500/30 w-full flex flex-col">
      <div className="w-full bg-[#002b5c] relative overflow-hidden shrink-0">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 md:py-4 relative z-10">
          ...
          <h1 className="text-[40px] leading-tight font-extrabold text-white tracking-tight">Site Map</h1>
        </div>
      </div>
      ...
      <div className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 pt-3 pb-8 md:pt-5 md:pb-12">
        <div className="bg-white rounded-3xl shadow-[0_8px_40px_rgb(0,0,0,0.03)] border border-slate-100/60 px-6 pt-3 pb-6 sm:px-10 sm:pt-4 sm:pb-10 md:px-14 md:pt-4 md:pb-8">
          <div className="mb-8">
            ...
            <h2 className="text-3xl md:text-4xl font-extrabold text-[#002b5c] mb-4 tracking-tight">Site Map</h2>
          ...
          <div className="space-y-4 max-w-3xl border-l border-slate-100 pl-4 md:pl-8 ml-2">
            ...
            {allSections.map((section, idx) => (
              <div key={idx} className="flex flex-col relative">
                ...
                <div className={`absolute -left-[29px] md:-left-[45px] top-[12px] w-2 h-2 rounded-full transition-colors ${openSections[section.title] ? "bg-[#007a87]" : "bg-slate-200"}`} />
                <button
                ...
                <div className={`overflow-hidden transition-all duration-300 ease-in-out ${openSections[section.title] ? "max-h-[2000px] opacity-100 mt-2 mb-4" : "max-h-0 opacity-0"}`}>
