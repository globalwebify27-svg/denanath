...
export default function SponsorsCROsClientPage({ pageData }: { pageData: any }) {
  const options = [
    {
        "name": "About Us",
        "href": "/research-about",
            ...
            <div className="bg-white rounded-3xl shadow-[0_8px_40px_rgb(0,0,0,0.03)] border border-slate-100/60 px-6 pt-3 pb-6 sm:px-10 sm:pt-4 sm:pb-10 md:px-14 md:pt-4 md:pb-8">
              ...
              {pageData.image && (
                <div className="mb-10 rounded-2xl overflow-hidden border border-slate-200">
                  <img src={pageData.image} alt={pageData.title || "Sponsors & CROs"} className="w-full h-auto object-contain max-h-[500px] bg-slate-50" />
                </div>
              )}
              
              {pageData.content ? (
                <div className="text-slate-600 text-base leading-[31px] font-normal [&_p]:mb-4 [&_ul]:list-disc [&_ul]:pl-5 [&_ul]:mb-4 [&_li]:mb-2 [&_p]:!text-base [&_p]:!leading-[31px] [&_p]:!font-normal [&_li]:!text-base [&_li]:!leading-[31px] [&_li]:!font-normal [&_strong]:!font-bold [&_b]:!font-bold [&_h1]:!font-bold [&_h2]:!font-bold [&_h3]:!font-bold [&_h4]:!font-bold" dangerouslySetInnerHTML={{ __html: pageData.content }} />
              ) : (
