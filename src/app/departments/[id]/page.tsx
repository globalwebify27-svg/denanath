...
export default async function DepartmentDetailsPage({
        ...
        if (drAhisRes.ok) {
          ...
          if (Array.isArray(apiDocs) && apiDocs.length > 0) {
            const circlesHtml = apiDocs.map((doc: any) => {
              ...
              // Always try to match against local DB doctors to get their uploaded images and details
              const nameForMatch = cleanName.toLowerCase().replace(/^dr\.?\s*/i, '').trim();
              const matchedLocalDoc = localDoctors.find((localDoc) => {
                if (profileDoctorId && localDoc.dmhDoctorId === profileDoctorId) return true;
              ...
              return `
                <a href="${doctorProfileUrl}" class="p-3.5 sm:p-5 bg-white border border-slate-200 rounded-2xl flex items-center justify-between gap-2.5 sm:gap-4 shadow-sm hover:shadow-md transition-shadow overflow-hidden group block">
                  <div class="flex items-center gap-2.5 sm:gap-4 flex-1 min-w-0 pr-1">
                    ...
                    <div class="text-sm sm:text-base font-bold text-[#002b5c] m-0 leading-snug group-hover:text-[#007a87] transition-colors min-w-0 break-words flex-1">${displayName}</div>
                  </div>
                  <div class="px-2.5 py-1.5 sm:px-4 sm:py-2 bg-teal-50 group-hover:bg-teal-100 text-[#007a87] rounded-xl text-[11px] sm:text-xs font-bold transition-colors shrink-0 ">
                    View Profile
                  </div>
                </a>
              `;
    ...
    $('section').each((_, section) => {
      ...
      if (h3Text === 'facilities') {
        ...
        if (wrappers.length > 0) {
          ...
          // Group adjacent h5/h6 and p tags into a single div so they stay together in the CSS grid
          $(section).find('h5, h6').each((_, h) => {
            const $h = $(h);
            const $next = $h.next();
            if ($next.is('p')) {
              const wrapper = $('<div class="flex flex-col h-full gap-2"></div>');
              $h.wrap(wrapper);
              $h.parent().append($next);
            }
          });
        }
               ...
               docCards.push(`
                <a href="${doctorProfileUrl}" class="p-3.5 sm:p-5 bg-white border border-slate-200 rounded-2xl flex items-center justify-between gap-2.5 sm:gap-4 shadow-sm hover:shadow-md transition-shadow overflow-hidden group block">
                  <div class="flex items-center gap-2.5 sm:gap-4 flex-1 min-w-0 pr-1">
                    ...
                    <div class="text-sm sm:text-base font-bold text-[#002b5c] m-0 leading-snug group-hover:text-[#007a87] transition-colors min-w-0 break-words flex-1">${name}</div>
                  </div>
                  <div class="px-2.5 py-1.5 sm:px-4 sm:py-2 bg-teal-50 group-hover:bg-teal-100 text-[#007a87] rounded-xl text-[11px] sm:text-xs font-bold transition-colors shrink-0 ">
                    View Profile
                  </div>
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] font-sans selection:bg-teal-500/30">
      ...
      <div className="max-w-[1300px] mx-auto px-4 sm:px-6 lg:px-8 pt-2 pb-8 md:pt-4 md:pb-12">
        ...
        <div className="flex flex-col lg:flex-row gap-6 xl:gap-8 items-start">
          ...
          <div className="w-full flex-1 min-w-0">
            <div className="bg-white rounded-3xl shadow-[0_8px_40px_rgb(0,0,0,0.03)] border border-slate-100/60 px-6 pt-3 pb-6 sm:px-10 sm:pt-5 sm:pb-10">
              ...
              <div className={`text-slate-700 space-y-4 break-words [&_img]:max-w-full [&_img]:h-auto [&_img]:rounded-2xl [&_img]:shadow-md [&_img]:my-4 [&_img]:mx-auto [&_img]:block md:[&_img]:inline-block md:[&_img]:mx-0 ${department.name.toLowerCase().includes('rheumatology') ? 'md:[&_img]:mr-4 md:[&_img]:mb-4' : ''} [&_p:not(:last-child)]:mb-2.5 [&_section:not(:last-child)]:mb-5
                ...
                [&_.facilities-images-grid_span]:block [&_.facilities-images-grid_span]:p-3 [&_.facilities-images-grid_span]:!bg-white [&_.facilities-images-grid_span]:w-full [&_.facilities-images-grid_span]:empty:hidden [&_.facilities-images-grid_span]:text-[#002b5c] [&_.facilities-images-grid_span]:!mt-auto [&_.facilities-images-grid_span]: [&_.facilities-images-grid_span]:!font-extrabold [&_.facilities-images-grid_span]:leading-snug
                
                [&_.grid_div_p]: [&_.grid_div_p]:!font-extrabold [&_.grid_div_p]:!mt-3
          ...
          <div className="w-full lg:w-[260px] xl:w-[280px] shrink-0 sticky top-24 space-y-6">
