...
export default async function AdminAccreditationsPage() {
  ...
  try { 
    if (setting) {
      ...
      accreditationsData = Array.isArray(fullData) ? fullData : (fullData.items || []);
    }
  } catch (e) {}
  if (!accreditationsData || accreditationsData.length === 0) {
    accreditationsData = [
    {
      id: 1,
  ...
  async function saveAccreditations(formData: FormData) {
    ...
    try {
      ...
      const existingSetting = await prisma.siteSetting.findUnique({ where: { key: 'page_accreditations' } });
