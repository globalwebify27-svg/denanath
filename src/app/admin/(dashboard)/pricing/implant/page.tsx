import { Stethoscope, Save, Search } from "lucide-react";
import { prisma } from "@/lib/prisma";
import { revalidatePath } from "next/cache";
import SubmitButton from "@/app/admin/(dashboard)/components/SubmitButton";
import ImplantClientForm from "./client-form";

export const dynamic = "force-dynamic";

export default async function AdminImplantPricingPage() {
  ...
  async function savePricingData(formData: FormData) {
    ...
    try {
      ...
      await prisma.siteSetting.upsert({
        ...
        create: { key: 'page_implant_pricing', value: finalJson }
      });
      revalidatePath("/admin/pricing/implant");
      revalidatePath("/implant-pricing");
    } catch (e) {
      console.error("Invalid JSON provided for implant pricing");
    }
  }
        ...
        <div className="mb-10 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 bg-white p-6 md:p-10 rounded-3xl shadow-sm border border-slate-100 relative overflow-hidden group">
          ...
          <div className="z-10 relative">
            ...
            <p className="text-[15px] font-medium text-slate-500 max-w-xl leading-relaxed">
              Manage the implant pricing table, including various systems, components, and brand rates.
            </p>
          </div>
          ...
          <div className="absolute right-0 top-0 opacity-[0.03] pointer-events-none group-hover:scale-110 transition-transform duration-700">
             <Stethoscope size={200} className="text-[#007a87] -mt-10 -mr-10" />
          </div>
        </div>

        <ImplantClientForm initialData={pricingData} />
