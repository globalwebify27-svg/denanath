...
export default function DepartmentDetailsClientForm({ initialData }: { initialData: any }) {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch("/api/settings", {
        ...
        body: JSON.stringify({
          key: "page_doctors_department_details",
          value: JSON.stringify(data),
          pathsToRevalidate: [
            "/admin/doctors/department-details",
            "/department-details"
          ]
        })
      });
      ...
      <div className="mb-10 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 bg-white p-6 md:p-10 rounded-3xl shadow-sm border border-slate-100 relative overflow-hidden group">
        ...
        <div className="z-10 relative">
          ...
          <p className="text-[15px] font-medium text-slate-500 max-w-xl leading-relaxed">
            Manage content for Department Details
          </p>
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-200">
            {data.image && (
              <div className="shrink-0 relative group">
                <img src={data.image} alt="Department Details" className="w-32 h-20 object-cover rounded-xl border border-slate-200 shadow-sm" />
                <button 
                  type="button" 
