...
import { Calendar, MapPin, Users, Award, ChevronRight, ArrowLeft, Target, Stethoscope, Briefcase, Activity, CheckCircle2, X, Play, Pause, ChevronLeft } from 'lucide-react';
...
export default function EventsClientPage({ events = [], pageData }: { events: any[], pageData?: any }) {
  ...
  useEffect(() => {
    if (isPlaying && galleryState) {
      const timer = setInterval(() => {
        setGalleryState(prev => prev ? { ...prev, currentIndex: (prev.currentIndex + 1) % prev.images.length } : null);
      }, 3000);
      return () => clearInterval(timer);
    }
  }, [isPlaying, galleryState]);
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc]">
      ...
      <div className="w-full bg-[#002b5c] relative overflow-hidden py-4">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <nav className="flex items-center gap-2 text-[#b2dfdb] mb-2 font-medium" style={{ fontSize: '10px' }}>
            ...
            <span className="text-white">{pageData?.pageTitle || "Events/News"}</span>
          </nav>
          
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="max-w-3xl">
              <h1 className="text-[40px] leading-tight font-extrabold text-white tracking-tight">
                {pageData?.pageTitle || "Events / News"}
              </h1>
            </div>
            
            <Link 
              href="/events" 
              className="inline-flex items-center gap-2 px-6 py-2.5 bg-white text-[#005f6b] rounded-xl hover:bg-[#e0f2f1] hover:scale-105 transition-all duration-300 font-bold shadow-lg shrink-0"
            >
                    ...
                    <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
                      ...
                      <div className="flex flex-col gap-4">
                        {gallery.map((img: string, idx: number) => (
                          <div 
                            key={idx} 
                            className="relative rounded-xl overflow-hidden group cursor-pointer aspect-video w-full shadow-sm"
                            onClick={() => {
                              const images = gallery.map((i: string) => i.startsWith('/') || i.startsWith('http') ? i : `/images/${i}`);
                              setGalleryState({ images, currentIndex: idx });
                            }}
                          >
                            <img 
      ...
      {galleryState && (
        <div 
          className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-black/95 backdrop-blur-sm animate-fadeIn"
        >
          <div className="relative w-full flex-1 flex items-center justify-center p-4 min-h-0" onClick={() => { setIsPlaying(false); setGalleryState(null); }}>
            <img 
          ...
          <div className="w-full max-w-4xl bg-[#f8f9fa] flex items-center justify-between px-6 py-4 mx-4 mb-6 rounded-md shadow-lg" onClick={(e) => e.stopPropagation()}>
            <div className="text-slate-700 font-bold text-sm tracking-wide w-24">
              {galleryState.currentIndex + 1} / {galleryState.images.length}
            </div>
            
            <div className="flex items-center gap-8">
              <button 
                className="text-[#002b5c] hover:text-[#007a87] transition-colors"
                onClick={() => setGalleryState(prev => prev ? { ...prev, currentIndex: (prev.currentIndex - 1 + prev.images.length) % prev.images.length } : null)}
              >
              ...
              <button 
                className="text-[#002b5c] hover:text-[#007a87] transition-colors"
                onClick={() => setGalleryState(prev => prev ? { ...prev, currentIndex: (prev.currentIndex + 1) % prev.images.length } : null)}
              >
            ...
            <div className="w-24 flex justify-end">
              <button 
                ...
                onClick={() => { setIsPlaying(false); setGalleryState(null); }}
              >
                <X className="w-6 h-6" />
              </button>
            </div>
