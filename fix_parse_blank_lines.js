...
async function run() {
  ...
  // Read the current DB to get header and footer
  ...
  const startIdx = currentData.content.indexOf('<div class="bg-white border border-slate-200');
  let startInner = currentData.content.indexOf('<h4 class="text-base md:text-lg font-bold');
  if (startInner === -1 || startInner > startIdx) startInner = startIdx;
  const header = currentData.content.substring(0, startInner);

  const footerIdx = currentData.content.lastIndexOf('          </div>\n        </div>\n\n        <div class="bg-slate-50 border border-slate-200 rounded-3xl');
  const footer = currentData.content.substring(footerIdx);
  ...
  const payload = {
    title: "Publications",
    content: header + finalHtml + footer
  };
  ...
  console.log('Total pubs:', parsedItems.filter(i=>i.type==='pub').length);
}
