...
const raw = fs.readFileSync('new_2024_pubs.txt', 'utf8');
...
while (i < lines.length) {
  ...
  chunk.push(lines[i]);
  i++;
  while (i < lines.length && !lines[i].match(/\([A-Za-z]+[- ]?[A-Za-z]* \d{4}\)/)) {
    chunk.push(lines[i]);
    i++;
  }
  ...
  if (chunk.length >= 2) {
    ...
    // Clean up trailing brackets if any
...
async function main() {
  ...
  for (let j = 0; j < dbLines.length; j++) {
    if (dbLines[j].includes('Publications: April 2024 \u2013 March 2025')) {
      startIdx = j;
    }
    if (dbLines[j].includes('Publications: April 2023 \u2013 March 2024') && startIdx !== -1) {
      endIdx = j;
      break;
    }
  }
  ...
  if (startIdx !== -1 && endIdx !== -1) {
    ...
    console.log("Updated 2024-2025 publications successfully. Replaced " + (endIdx - startIdx) + " lines with " + pubs.length + " pubs.");
  } else {
