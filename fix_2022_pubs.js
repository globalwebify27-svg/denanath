...
const raw = fs.readFileSync('new_2022_pubs.txt', 'utf8');
...
while (i < lines.length) {
  ...
  chunk.push(lines[i]);
  i++;
  // Regex matches things like (January 2023), (November - December 2022), (Oct – Dec 2022)
  while (i < lines.length && !lines[i].match(/\([A-Za-z]+\s*(?:[-–]\s*[A-Za-z]+)?\s*\d{4}\)/)) {
    chunk.push(lines[i]);
    i++;
  }
...
async function main() {
  ...
  for (let j = 0; j < dbLines.length; j++) {
    if (dbLines[j].includes('Publications: April 2022 \u2013 March 2023')) {
      startIdx = j;
    }
    if (dbLines[j].includes('Publications: April 2021- March 2022') && startIdx !== -1) {
      endIdx = j;
      break;
    }
  }
  ...
  if (startIdx !== -1 && endIdx !== -1) {
    ...
    console.log("Updated 2022-2023 publications successfully. Replaced " + (endIdx - startIdx) + " lines with " + pubs.length + " pubs.");
  } else {
